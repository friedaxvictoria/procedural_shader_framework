...
void addDolphin_float(float index, float3 position, float timeOffset, float speed, float3 axis, float angle, float3 baseColorIn, float3 specularColorIn, float specularStrengthIn,
...
void raymarch_float(float condition, float numSDF, float2 uv, float3x3 camMatrix, out float4 hitPos, out float3 normal, out float3 rayDirection, out int hitIndex)
{
    if (condition == 0)
    {
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        hitIndex = bestID; // Store the ID of the closest hit shape
        d += _sdfNoise[hitIndex] * 0.3; // Evaluate the scene SDF at the current point, add noise
        if (d < 0.001)
        {
            hitPos.xyz = p;
            normal = get_normal(hitIndex, p);
            break;
        }
