...
float3 dolphinMovement(float time, float timeOffset, float3 basePosition, float speed)
{
    if (speed == 0)
        return basePosition;
    float adjustedTime = time + timeOffset;
...
float2 dolphinDistance(float3 p, float3 position, float timeOffset, float speed, float time)
{
    ...
    float3 startPoint = dolphinMovement(time, timeOffset, position, speed);
    ...
    for (int i = 0; i < NO_OF_SEGMENTS; i++)
    {
        ...
        float2 segmentAnimation = speed == 0 ? float2(0, 0): dolphinAnimation(segmentPosition, time, timeOffset);
...
float3 getDolphinColor(float3 hitPos, float3 normal)
{
    ...
    float3 BRDF = 20.0 * 1 * float3(4.00, 2.20, 1.40);
    material *= BRDF;
    return material;
}
...
float evalSDF(int i, float3 p)
{
    ...
    if (sdfType == 0)
        ...
        return dolphinDistance(probePt, _sdfPositionFloat[i], _timeOffsetDolphinFloat[i], _speedDolphinFloat[i], 0.6 + 2.0 * _Time.y - 20.0).x;
    else if (sdfType == 4)
